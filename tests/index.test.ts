...
import { APIPromise } from 'conductor-node/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['CONDUCTOR_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Conductor) => {
    ...
    test('default logLevel is warn', async () => {
      ...
      expect(conductor.logLevel).toBe('warn');
    });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const conductor = new Conductor({ logger: logger, apiKey: 'sk_conductor_...' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'CONDUCTOR_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('withOptions', () => {
    test('creates a new client with overridden options', async () => {
      ...
      const newClient = conductor.withOptions({
        maxRetries: 5,
      ...
      expect(conductor.maxRetries).toEqual(3);
      expect(conductor.baseURL).toEqual('http://localhost:5000/');

      // Verify it's a different instance
      expect(newClient).not.toBe(conductor);
      expect(newClient.constructor).toBe(conductor.constructor);
    });
    ...
    test('respects runtime property changes when creating new client', () => {
      ...
      conductor.baseURL = 'http://localhost:6000/';
      conductor.timeout = 2000;
      ...
      expect(conductor.baseURL).toEqual('http://localhost:6000/');
      expect(conductor.timeout).toEqual(2000);
      expect(conductor.maxRetries).not.toEqual(10);
