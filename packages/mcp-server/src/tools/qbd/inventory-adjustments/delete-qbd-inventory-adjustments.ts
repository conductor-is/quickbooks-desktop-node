...
export const handler = async (conductor: Conductor, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await conductor.qbd.inventoryAdjustments.delete(id, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
