...
export interface ClientOptions {
   ...
   * Set the log level.
   *
   * Defaults to process.env['CONDUCTOR_LOG'] or 'warn' if it isn't set.
   */
...
export class Conductor {
  ...
  constructor({
    ...
    apiKey = readEnv('CONDUCTOR_SECRET_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
    ...
    this.timeout = options.timeout ?? Conductor.DEFAULT_TIMEOUT /* 2 minutes */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CONDUCTOR_LOG'), "process.env['CONDUCTOR_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('CONDUCTOR_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  >(
    ...
    return new Pagination.PagePromise<PageClass, Item>(this as any as Conductor, request, Page);
  }
